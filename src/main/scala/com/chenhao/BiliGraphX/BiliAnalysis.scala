...
object BiliAnalysis {
  def main(args: Array[String]): Unit = {
    ...
    val filepath = "/Users/chenhao/Documents/Data/GraphxData/bili-1"
...
//    val topKFans = rankByFans.sortBy(t=>t._2._2,ascending = false).take(K)
//    topKFans.foreach(t=>{
...
//    val topKUp = rankByUser.sortBy(v => v._2._2, ascending = false).take(K)
////  将前几个最受欢迎的推送到数据库
...
//    println(cc.filter(x=>x._2.size>=10).count())
    println("社区中人数有5-10")
    val showCC = cc.filter(x=>x._2.size>=5&&x._2.size<=10).cache()
//    showCC.foreach(t=>println(t._1.toString))
    showCC.collect.foreach(t=>{
      val cmId = t._1
//      println("------社区ID"+cmId.toString)
      ccVertices.filter(n=>n._2==cmId).foreach(m=>{
        val src = m._1
...
//      r.foreach(mn => {
//        println(mn)
//        val e = MongoDBObject("Source" -> mn.get("Source"), "Target" -> mn.get("Target"))
//        db("comu").insert(e)
//      })
//
...
//    val num:Int = (edgesComu.count() * 0.01).toInt
//    subGraph1.edges.collect.take(num)foreach(println(_))
